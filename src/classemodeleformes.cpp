...
classeModeleFormes::classeModeleFormes(QObject *parent) : QAbstractListModel(parent),
		fenetrePrincipale(qobject_cast<classePrincipale*>(parent))
{
}

int classeModeleFormes::rowCount(const QModelIndex &) const
{
	return fenetrePrincipale->CompterFormesFlechies();
}

QVariant classeModeleFormes::data(const QModelIndex &index, int role) const
{
	...
	return fenetrePrincipale->LireFormeFlechie(index.row());
}
