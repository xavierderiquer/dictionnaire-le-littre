...
classeNavigation::classeNavigation(bool boolModeAccessible, QWidget *parent,
		Qt::WindowFlags f) : QDialog(parent, f), nombreEntreeChoisie(-1)
{
	setupUi(this);
	setFixedSize(size());

	connect(boutonConsulter, SIGNAL(clicked()), this, SLOT(Consulter()));
	connect(zoneListeCorrespondances, SIGNAL(activated(QModelIndex)), this,
			SLOT(Consulter()));
...
void classeNavigation::EvenementAccessibleZoneListe(const QModelIndex &index)
{
	QListView *zoneListe(qobject_cast<QListView *>(sender()));
	zoneListe->viewport()->setAccessibleName(index.data().toString());
}

void classeNavigation::Consulter()
{
	nombreEntreeChoisie = zoneListeCorrespondances->
