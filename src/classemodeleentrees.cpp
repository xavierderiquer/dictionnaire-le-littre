...
classeModeleEntrees::classeModeleEntrees(QObject *parent) : QAbstractListModel(parent),
		fenetrePrincipale(qobject_cast<classePrincipale*>(parent))
{
}

int classeModeleEntrees::rowCount(const QModelIndex &) const
{
	return fenetrePrincipale->CompterEntrees();
}

QVariant classeModeleEntrees::data(const QModelIndex &index, int role) const
{
	...
	return fenetrePrincipale->LireEntree(index.row());
}
