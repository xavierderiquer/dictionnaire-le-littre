...
classeAPropos::classeAPropos(bool boolModeAccessible, QWidget *parent, Qt::WindowFlags f)
		: QDialog(parent, f)
{
	setupUi(this);
	setFixedSize(size());
	...
	foreach(QTextBrowser *navigateur, findChildren<QTextBrowser *>()) {
		navigateur->setPalette(palette);
		if(boolModeAccessible)
			navigateur->viewport()->setAccessibleName(navigateur->toPlainText());
	}
}
